...
- [x] **1-6** 녹화 버튼: 누르는 동안 녹화 / 떼면 종료
    - `Button`은 탭이 끝나야 동작하므로 쓸 수 없다. `DragGesture(minimumDistance: 0)`의 `onChanged`/`onEnded`로 처리한다
    - 10초 한도로 이미 끝난 뒤 손을 떼면 `stopRecording()`이 가드에서 반환한다
- [x] **1-7** 1초 미만 클립 폐기
