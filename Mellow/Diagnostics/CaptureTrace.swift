...
final class CaptureTrace: @unchecked Sendable {
    ...
    /// 정작 봐야 할 로그가 묻힌다.
    ///
    /// **릴리스에서 빠지는 것은 이 상수 하나뿐이다.** `false` 로 고정되어
    /// 각 메서드가 즉시 반환하므로 동작은 꺼지지만, **타입·메서드·로그
    /// 문자열은 Release 이진에 그대로 남는다** (심볼 38건, 2-A 에서 이진
    /// 확인). 파일 전체를 `#if DEBUG` 로 감싼 `StoreProbeLog` 와 다르다.
    ///
    /// 이 구분을 적어두는 이유는 "`CaptureTrace` 는 릴리스에 안 들어가니
    /// 괜찮다" 를 근거로 판단하면 틀리기 때문이다. 파일 전체로 감쌀지는
    /// 3-13 에서 정한다.
    ///
