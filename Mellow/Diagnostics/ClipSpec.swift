...
extension ClipSpec {
    ...
    static func differences(base: ClipSpec, other: ClipSpec) -> [SpecDifference] {
        ...
        case let (lhs?, rhs?):
            ...
            //
            // minFrameDuration 이 낮게 나오는 경로가 둘 확인됨(iPhone 12 실측):
            //   (1) 트랙 길이가 길수록 (표본 크기 효과)
            //       — 7~10초 20/600, 60~188초 19/600
            //   (2) 저조도 촬영 (실제 fps 저하)
            //       — 같은 9.94초라도 밝은 곳 20/600, 저조도 19/600
            // 원인이 둘이므로 호환성 판정 기준으로 부적절하다는 근거가 강화됨.
            // nominalFrameRate 를 정보 항목에 함께 둔 것이 두 경로를 구분하는
            // 수단이 되었다 — (1)은 minFrameDuration 만 내려가고,
            // (2)는 nominalFrameRate 도 함께 내려간다(30.0 → 29.995).
            //
            // 단, 이 판단은 1-12 병합에서 지터가 다른 클립이 실제로 재인코딩 없이
            // 붙는 것이 확인되어야 확정이다.
            //
            // nominalFrameRate 자체도 Float 정밀도 탓에 같은 30000/1001 이
            // 파일마다 마지막 자리가 갈린다. 값 확인용으로만 쓴다.
