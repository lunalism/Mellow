...
/// .cube LUT로 렌더한다(`lutFiltered` → `LUTStore.loadedCube` + `LUTFilter.apply` —
/// 라이브 프리뷰와 **동일한 LUT**) — 필터를 파일에 베이크하지 않고, 두 번째 필터 구현도
/// 만들지 않는다. 썸네일 색은 LUT 로드가 완료된 프리뷰와 일치한다 — 선택된 slug의 LUT가
/// 아직 로드되지 않아 프리뷰가 패스스루로 표시되는 동안 촬영한 캡처는 예외(저장·재렌더는
/// LUT가 적용된다).
///
...
final class CaptureThumbnailRenderer {
    ...
    /// **원본 전체**(예: 2268×4032)를 디코드해, 프리뷰·썸네일과 **동일한 .cube LUT**(`lutFiltered`)로 렌더한다
    /// → LUT 로드가 완료된 상태의 프리뷰와 같은 LUT 데이터·색, 풀해상도, 워터마크 없음. **백그라운드에서 호출.**
    ///
