...
ADR-043에 따라 가로 영상(preferredTransform 적용 후 가로 > 세로)은 V1에서 지원하지 않는다: 여러 영상 선택(Select Clips / Add)에서는 가로 영상만 제외하고 세로 영상으로 계속 진행하며 가로가 유일한 제외 사유면 `가로 영상이 제외되었어요` / `세로 영상만 추가할 수 있어요.`를 한 번 보여주고, 모두 가로면 프로젝트를 만들거나 바꾸지 않는다. Replace 후보가 가로 영상이면 `가로 영상은 사용할 수 없어요` / `세로 영상을 선택해주세요.`로 거부하고 기존 Clip을 그대로 둔다. 가로 영상에는 어떤 Media 작업도 하지 않는다. 일반 iPhone 세로 촬영본(자연 크기 1920×1080이지만 세로로 표시되는 영상)은 세로 영상이다. 이 동작은 승인된 정책이며 아직 구현되지 않았다(현재 Phase 5는 가로 영상을 만나면 선택 전체를 거부한다).
...
- Imported Video의 Aspect mismatch 기본 정책은 Fill + Crop이며 사용자가 Framing 위치를 조정할 수 있어야 한다(ADR-043: 세로 Presentation 영상의 비율 불일치에 한하며 가로 영상은 V1 Photos Import에서 제외된다).
- Fit과 Background Blur는 MVP에서 제공하지 않는다.
