...
ADR-044에 따라 V1 Photos Import는 iPhone 촬영 → Photos 선택 → Mellow 준비 Workflow를 위한 것이며 실제 Container가 QuickTime Movie인 영상만 받아들인다(H.264 / HEVC 모두). 가져올 수 있는 영상의 Video Codec은 H.264와 HEVC뿐이며 ProRes / ProRes RAW / Motion JPEG 등 그 밖의 Codec 영상은 같은 "읽을 수 없거나 지원하지 않는 영상" 안내로 제외된다(ADR-046; Codec별 문구 없음, 변환 없음). MP4 등 QuickTime이 아닌 영상은 파일 이름 확장자가 아니라 실제 Container 검사로 판별해 제외하며(`.mp4`를 `.mov`로 바꿔도 허용되지 않고, 진짜 QuickTime 파일은 `.mp4` 이름이어도 제외되지 않는다) 변환 · Remux · 변환 안내를 제공하지 않는다. 여러 영상 선택에서는 해당 영상만 제외하고 QuickTime 영상으로 계속하며 안내는 기존 `일부 영상을 추가할 수 없어요` / `읽을 수 없거나 지원하지 않는 영상은 제외되었어요.`(길이 / 세로 형식 사유와 복합이면 `일부 영상이 제외되었어요` / `길이 조건에 맞지 않거나 사용할 수 없는 영상은 추가할 수 없어요.`)를 쓴다. 모두 제외되면 프로젝트를 만들거나 바꾸지 않는다. 단일 선택이나 Replace 후보가 읽을 수 없거나 지원하지 않는 영상이면 `영상을 추가할 수 없어요` / `읽을 수 없거나 지원하지 않는 영상이에요. 다른 영상을 선택해주세요.`를 보여주고(ADR-044 Revision 1) Replace는 기존 Clip · Media · 순서를 그대로 둔다. QuickTime이라는 사실만으로 자동 허용되지는 않으며 길이 · 세로 · 읽기 가능 등 다른 규칙을 모두 통과해야 한다. 이 동작은 승인된 정책이며 아직 구현되지 않았다(현재 Phase 5는 Container를 검사하지 않는다).
...
- 직접 촬영 Clip은 자유롭게 촬영하며 최대 5초다. Mellow가 직접 촬영한 영상은 항상 같은 형식(QuickTime `.mov`, H.264, SDR, 1080p 30 fps)으로 저장되며 사용자가 Codec을 고르지 않는다(ADR-046).
