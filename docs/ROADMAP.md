...
- **Logical Clip Delete + Editor Session Undo / Redo — 구현 2026-09-16 (Phase 5 STEP 10 revised per ADR-038, Physical Review Pending):** 선택 Clip의 명시적 Delete(Dock Trailing `trash`, 확인 없음) → 즉시 Active Timeline에서 제거 + Total 감소 + 결정적 Selection Fallback + Durable Pending Deletion Autosave. Undo / Redo는 Navigation Bar 우상단 상시 Control(`실행 취소` / `다시 실행`)의 시간순 LIFO Session History로 Reorder와 Delete를 되돌리고 다시 적용하며(각각 한 번 Autosave + Read-back, 새 편집은 Redo 폐기, 실패 시 State / Stack 불변), 이후 모든 Editor 편집이 같은 모델을 쓴다. Delete Undo / Redo는 같은 Clip UUID / Media / Metadata를 오가며 Duplicate를 만들지 않는다. Domain은 Active(`clips`) / Durable(`deletedClips`, `ClipDeletionRecord`)을 분리하고 `ProjectRepository.update`는 빠진 Clip을 암묵적으로 삭제하지 않으며(`missingDurableClip` 거부) 물리적 Metadata 제거는 명시적 `finalizeDeletedClip`만 가능하다(호출하지 않음). SwiftData Additive Optional Field 4개, Lightweight Migration 검증. Delete-only Snackbar / "가장 최근 삭제 한 건" / Undo Window Timer는 ADR-038로 폐기되었고 Undo Window Gate는 더 이상 존재하지 않는다. **Final Physical Cleanup(Session 종료 후 Pending Deletion의 ADR-021 안전 조건 기반 정리)은 STEP 12A(ADR-039)로 구현되었다.**

- **Deferred Physical Cleanup of Known Pending Clips — 구현 2026-09-16 (Phase 5 STEP 12A, ADR-039, Physical Review Pending):** `ProjectMediaCleanupCoordinator`가 `Project.deletedClips`만 대상으로 Editor Route 제거(Back, 비동기 — Navigation 무대기)와 App 시작(모든 Durable Project) 두 경계에서 Reconcile한다. Live Editor Session 동안 그 Project의 Pending Clip은 절대 정리하지 않으며(Undo / Redo / Redo Invalidation / Reorder / Delete / Add 어느 것도 Cleanup을 촉발하지 않음), 순서는 Canonical Path 완전 일치 → Thumbnail Consumer Idle(Bounded 3초, Timeout은 Defer) → File 삭제 → 부재 검증 → `finalizeDeletedClip`(`updatedAt` 불변)이다. Pending + File 없음은 Crash Recovery로 Finalize, Active + File 없음은 절대 미접촉, Clip별 실패 격리 / Idempotent Retry, 사용자 Error 없음. Cleanup ↔ Composition / Replacement ↔ Editor Project Load는 `ProjectLifecycleOperationGate`로 직렬화되어 Editor가 중간 Snapshot을 읽어 Finalize된 Clip을 되살릴 수 없다. **STEP 12B(Row 없는 Media / Project Directory / 버려진 ProjectWorkspace Sweep, ADR-020 Confirmed Orphan Contract)는 12A 실기기 검증 뒤 별도 Slice로 남는다.**

- **Add Clips to Current Project + Undo / Redo — 구현 2026-09-16 (Phase 5 STEP 11, Physical Review Pending):** Editor Leading `+`가 Production Action이 되어 Editor 전용 PhotosPicker Session → 기존 Phase-5-ready Validator / Admission / `ProjectMediaStore` → `ProjectClipAppendCoordinator`(Validate ALL → Materialize ALL, 실패 시 이 Operation 파일만 제거) → 현재 Project 끝에 Picker 순서 Append → 1회 Autosave + Read-back → 하나의 `.add` History Entry(첫 새 Clip 선택). Cancel / Non-ready / 실패는 All-or-nothing. Undo Add는 새 Clip을 Durable Pending(Inactive)으로 유지하고 Media를 보존하며 Redo는 같은 Identity / 파일을 되살린다; Undo 뒤 새 편집 / Session 종료로 Redo가 사라진 Clip은 STEP 12 Cleanup 대상으로 남는다. Camera 재진입 / Project 대체 / Phase-6 준비 없음.
...
10. Session History가 끝난 Pending Deletion의 Physical Media Cleanup은 Undo / Recovery / Active Usage / Late Commit 차단과 Safe Classification 조건을 모두 충족할 때까지 지연한다. — 구현 2026-09-16 (STEP 12A, ADR-039: Live Editor Session 동안 금지, Editor 종료 / App 시작 경계에서 File-first Reconcile, Timer 없음; Orphan / Workspace Sweep은 STEP 12B)
...
### Accepted Direction — ADR-022 / ADR-042 / ADR-043
...
Phase 6는 Physical iPhone 12에서 1080p SDR, 4K SDR, HDR / Dolby Vision, Portrait Aspect Mismatch를 포함한 5초 이하 전체 Portrait Source의 재현 가능한 Import / Normalization Baseline Measurement Evidence를 남긴다.
...
- Portrait Aspect Mismatch(예: 4:3 Portrait) → 9:16(ADR-043: Landscape → 9:16 시나리오 제거; Portrait → 16:9는 Landscape Project 복원 이후)
- Audio Sync 유지
...
5. 1080p SDR, 4K SDR, HDR / Dolby Vision, Portrait Aspect Mismatch의 5초 이하 전체 Portrait Source Import / Normalization의 Elapsed Observation, Memory, Peak Additional Storage, Thermal, Success / Failure와 applicable Cancellation Responsiveness를 Official Scenario로 실행한다.
