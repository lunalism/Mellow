...
- **STEP 12B — Orphan / Workspace Recovery (Pending, Not Started):** Row 없는 `Projects/<pid>/Media/*` 파일, Project Row 없는 `Projects/<pid>/` Directory, 버려진 `ProjectWorkspace/<op>/`의 안전한 분류 / 정리(ADR-020 Recovery Classification + Confirmed Orphan Contract). STEP 12A의 Physical Review 통과 뒤 시작한다.
