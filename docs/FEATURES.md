...
Mellow의 제품 방향에는 적합하지만 첫 번째 MVP에는 필수적이지 않은 기능이다.
...
새 프로젝트를 만들 때 프로젝트 이름을 먼저 입력하도록 요구하지 않는다.
...
앱을 종료하거나 다시 실행해도 저장된 Draft를 다시 열 수 있어야 한다.
...
실수로 삭제하는 상황을 줄이기 위한 UX는 `DESIGN.md`에서 결정한다.
...
사용자는 필요할 때 Rear Camera와 Front Camera를 전환할 수 있어야 한다.
...
사용자는 촬영 전 Rear Camera와 Front Camera를 전환할 수 있어야 한다.

녹화 중 Camera 전환 지원 여부는 초기 MVP 필수 요구사항으로 두지 않는다.
...
구체적인 Progress 표현과 10초 도달 전 시각적 피드백은 `DESIGN.md`에서 결정한다.
...
Photos에서 가져온 영상은 프로젝트에서 안정적으로 사용할 수 있도록 Mellow의 로컬 프로젝트 저장소에 안전한 복사본을 생성하는 방향을 사용한다.

사용자가 Photos에서 원본 영상을 삭제하더라도 이미 Mellow 프로젝트에 추가한 Draft가 쉽게 손상되지 않아야 한다.

Draft를 삭제할 때는 Mellow 내부 복사본만 삭제하며 Photos의 원본 영상에는 영향을 주지 않는다.

구체적인 미디어 저장 방식은 `ARCHITECTURE.md`에서 결정한다.
...
정확한 기본 동작과 사용자가 조정할 수 있는 범위는 `DESIGN.md`에서 결정한다.
...
초기 MVP에서는 Clip 사이에 특별한 Transition 효과를 요구하지 않는다.
...
10. 불필요한 Clip을 삭제할 수 있다.
11. Clip의 순서를 변경할 수 있다.
12. 각 Clip의 시작점과 종료점을 Trim할 수 있다.
13. 전체 Vlog를 Preview할 수 있다.
14. 하나의 영상으로 Export할 수 있다.
15. Export한 영상을 Photos Library에 저장할 수 있다.
16. 앱을 종료하고 다시 실행해도 미완성 프로젝트를 계속 작업할 수 있다.
...
- 녹화 중 Front / Rear Camera 전환 허용 여부
- Tap to Focus의 MVP 포함 여부
- Exposure Control의 MVP 포함 여부
- Zoom의 MVP 포함 여부
- Torch의 MVP 포함 여부
- Camera Lens 선택 정책
...
- 9:16 프로젝트 촬영 중 기기를 가로로 들었을 때의 사용자 안내
- 16:9 프로젝트 촬영 중 기기를 세로로 들었을 때의 사용자 안내

## Imported Video

- 다른 화면 비율의 영상을 가져왔을 때 기본 처리 방식
- Fill and Crop을 기본값으로 사용할지 여부
- Fit 선택을 제공할지 여부
- Background Blur 등의 처리 기능 필요 여부

## Project

- Home에서 `Recent` 또는 다른 명칭을 사용할지 여부
- Draft 삭제 전 Confirmation 방식
...
- Clip 삭제 Undo 제공 여부
- Clip Duplicate 기능 필요 여부
- Clip Split 기능 필요 여부
- 개별 Clip Mute 기능 필요 여부

## Export

- 기본 Export Resolution
- 기본 Frame Rate
- Video Codec
- HDR 처리 정책
- Export Quality 선택 기능 제공 여부
- Share Sheet를 MVP에 포함할지 여부
