...
- 단일 항목 / Replace 후보의 5.0초 초과 거부 안내는 기존 `영상이 너무 길어요` / `5초 이하의 영상을 선택해주세요.`, 1.0초 미만 거부 안내는 `영상이 너무 짧아요` / `1초 이상의 영상을 선택해주세요.`(ADR-042 Revision 2); 단일 항목 / Replace 후보가 Preflight 판별 Unreadable / Unsupported(non-QuickTime Container 포함)이면 `영상을 추가할 수 없어요` / `읽을 수 없거나 지원하지 않는 영상이에요. 다른 영상을 선택해주세요.`(ADR-044 Revision 1, 승인 · 미구현); Replace 후보 거부 시 기존 Clip · Media · Metadata · 순서 · Slot 보존
...
- 하나의 촬영 Clip은 최대 5초다.
- Mellow에서 직접 촬영한 Clip은 QuickTime `.mov` · H.264 · SDR · 1080p 30 fps로 기록된다(ADR-046; 내부 불변조건, 사용자 설정 아님; 현재 구현은 아직 Codec을 명시하지 않음).
