...
- Portrait Video
- Landscape Video(ADR-043 Revision 1: Landscape / Square 등 Non-portrait Presentation은 V1 Photos Import에서 Preflight 제외 — 구조적 허용 범위이지 V1 지원 입력이 아님)
...
비율이 다른 Portrait Source(예: 4:3 Portrait)를 9:16 Project에 가져온다는 이유로 Import 때 중앙 9:16 영역만 잘라 저장하지 않으며 이후 Framing에 필요한 Source 영역을 유지한다. (ADR-043 Revision 1: 16:9 Landscape나 1:1 Square Source는 V1 Import 대상이 아니므로 이전의 "16:9 Source → 9:16 Project" 예시는 적용되지 않는다.)
...
비율이 다른 Portrait Source(예: 4:3 Portrait)를 9:16 Project에 추가하면 Preview / Export Composition에서 9:16 Canvas를 가득 채운 후 초과 영역을 Crop한다. (ADR-043 Revision 1: Landscape 16:9 / Square 1:1 Source는 V1 Photos Import에서 Preflight 제외되므로 "16:9 Source → 9:16 Project" 예시는 V1에 존재하지 않는다.)
...
- **Failure:** Missing / Unreadable / No Frame / Generation 실패는 Typed `ClipThumbnailError`이며 Presentation `.unavailable`(중립 Placeholder)로만 표현된다. Clip Metadata, 순서, 선택, Persistence는 변경하지 않는다.
- **STEP 13 Derived Availability (ADR-040):** `Core/Projects/ClipAvailability.swift` — `ClipAvailability`(`.available` / `.unavailable(.mediaMissing | .unresolvablePath)`), `ClipAvailabilityChecking`, Production `CommittedMediaAvailabilityChecker`(같은 Read-only Resolver Wrapper; Resolver의 문서화된 `mediaMissing` / `pathEscapesRoot`만 Unavailable, 그 밖의 오류는 Available로 보존). `ProjectEditorModel`이 `availabilityByClipID`를 소유하고 `loadThumbnails` 앞에서 `refreshAvailability()`로 Active Clip 전부를 재파생한다(Generation + Identity / Path Guard). Presentation은 파생 우선: Unavailable이면 `.mediaUnavailable`, 아니면 기존 Thumbnail 상태. 알려진 Unavailable Clip은 Thumbnail 요청 대상에서 빠지고 늦은 결과는 `applyThumbnailResult`에서 버려진다. 영속 Field / Schema 변경 없음.
- **Ownership:** Service = Resolution / Generation / Cache, `ProjectEditorModel` = Per-clip Presentation State + Request Identity + Selection(Canonical), View = Loading / Image / Placeholder Rendering.
- **STEP 14 Representative Thumbnail Infrastructure + Projects-surface Presentation (ADR-034 §7, ADR-041, 2026-09-17):** `Features/Home/ProjectRepresentativeModel.swift`가 저장 Project의 파생 Representative State(`ProjectRepresentativeState`: `.noProject` / `.project(id, thumbnail?)`)를 소유한다. 선택 규칙은 기존 Domain Policy `RepresentativeThumbnailSource.clipID(in:isUsable:)`(현재 논리 순서의 첫 Active Clip)이고 Usability는 STEP 13 `ClipAvailabilityChecking`(논리 순서로 묻고 첫 Available에서 중단)이다 — Pending / Deleted Clip은 후보가 아니고 Unavailable Clip은 이 선택에서만 건너뛰며 Project는 절대 변형하지 않는다. Thumbnail은 같은 `ClipThumbnailProviding`(Project-owned Committed Media, Midpoint Frame, Cache / Coalescing)을 Projects Visual Pixel Budget(80 × 80pt × 3)으로 요청한다. **Stale-result Protection:** 결과는 도착 시점의 `currentRequest`(Project + Clip Identity + Media Path + Trim Range + Pixel Budget)와 정확히 일치할 때만 발행되고, 매 Refresh가 Repository에서 그 Request를 다시 파생하므로 Reorder / Delete / Undo / Redo / Replace / Availability 변화 / Safe Atomic Replacement / Project 삭제 뒤의 늦은 결과는 Identity로 거부된다(Index 아님). Request Identity는 현재 모델링된 Effective Edit State(Trim Range)를 포함하며 Phase 7이 Framing / Edit Revision을 실제로 도입할 때 같은 Request 경계를 확장한다(Index Key 없음). Refresh Generation은 선택 발행만 보호한다. Refresh Trigger는 App 시작, Scene Active, Editor Route가 없는 Navigation 변경(Projects / Recent / Root 복귀)이며 Timer / Polling은 없고 Project Read는 `ProjectLifecycleOperationGate` 뒤에서 일어난다. 소비자는 Projects 화면 Visual(`ProjectsEntryView(representativeImage:)`)뿐이며 Camera Content Slot은 ADR-041에 따라 소비하지 않는다. 영속 Field / Schema 변경 없음; Thumbnail 실패는 Placeholder일 뿐 Availability / Project를 바꾸지 않는다. DEBUG Seam: `-uiTestRepresentativeDiagnostics`.
...
이 Gate가 해결되기 전에는 Replacement Metadata Migration을 구현하지 않는다. — Resolved by ADR-040 (2026-09-16, Phase 5 STEP 13): 새 Clip Identity(Model B)가 같은 Logical Slot을 차지하고 Trim Reset / Framing nil / 새 Thumbnail; 구현 계약은 60절 "Phase 5 STEP 13 Implementation — Replace Transaction".
...
Phase 6 소유 준비(4K → 1080p, HDR → SDR, Frame-rate Normalization, Portrait Source의 Transform Bake; Long-source Segment는 ADR-042로, Non-portrait(Landscape / Square) Source는 ADR-043 Revision 1로 제외)는 이 계약에 포함되지 않으며 Camera Recording은 계속 Photos에만 저장되고 어떤 Project에도 자동으로 붙지 않는다.
...
Phase 5 STEP 11 Implementation: Add Transaction은 `ProjectEditorModel.addClips()`가 소유하는 하나의 직렬화된 Editor Mutation이다(`isAddingClips` 동안 Reorder / Delete / Undo / Redo / 두 번째 Picker 거부). 순서: Workspace → Editor 전용 `PhotosVideoSelector`(Session-isolated Cancel, Per-file Pre-copy Admission) → `ProjectClipAppendCoordinator.prepareClips(for:sources:)`(Reserve Guard → Validate ALL → Materialize ALL into `Projects/<현재 pid>/Media/<cid>.mov`, 실패 시 이 Operation의 파일만 `removeMedia`) → `VlogProject.appendClips` → `commitEdit(.add)`(Persist 1회 + Read-back) → Workspace 폐기. Persist 실패는 이전 State / Selection 복귀 + 새 파일 제거(기존 Media 불변, Pending Deletion 생성 없음). **History-retained Media Lifetime:** Undo Add는 새 Clip을 Pending-deleted(Inactive, Durable, 위치 Anchor 기록)로 유지하고 파일을 지우지 않으며 Redo Add는 같은 Row / 파일을 Active로 되돌린다; Session 종료 후 남은 Undone-Add Clip은 재표시되지 않고 Durable Pending으로 남아 STEP 12 Physical Cleanup의 대상이다(`finalizeDeletedClip` 미호출).
