...
ADR-043 Revision 1에 따라 세로 형식이 아닌 영상(preferredTransform 적용 후 `presentationHeight > presentationWidth`가 아닌 가로 / 정사각형 영상)은 V1 미지원이며 Preflight에서 제외된다. 가로와 정사각형을 구분한 안내는 없고 하나의 Non-portrait Copy만 사용한다: 다중 선택에서 세로 형식이 아닌 영상만 제외되었으면 `일부 영상이 제외되었어요` / `세로 형식이 아닌 영상은 추가할 수 없어요.`를 한 번(개수 표시 없음), 단일 후보 선택 / Replace 후보가 세로 형식이 아니면 `지원하지 않는 영상이에요` / `세로 영상을 선택해주세요.`를 표시하고 기존 Clip을 보존하며, 길이 / Invalid 복합 사유는 `일부 영상이 제외되었어요` / `길이 조건에 맞지 않거나 사용할 수 없는 영상은 추가할 수 없어요.`를 사용한다. 취소 / 실패는 여전히 제외 안내보다 우선한다. 일반 iPhone 세로 촬영본은 자연 크기가 가로여도 세로 영상이다. 이 안내는 승인되었으나 아직 구현되지 않았다.
...
예를 들어 4:3 세로 영상을 9:16 프로젝트에 가져오면 9:16 Canvas를 채우도록 확대하고 필요한 영역을 Crop한다. (ADR-043 Revision 1: 16:9 가로 영상과 1:1 정사각형 영상은 V1 Photos Import에서 Preflight 제외되므로 이전의 가로 영상 예시는 적용되지 않는다.)
...
Phase 5 STEP 9 Reorder Interaction(V4.1 Timeline, 2026-09-16 구현 — Physical Review Pending): Short Tap은 선택만 한다(절대 Drag를 시작하지 않는다). Long Press **0.4초**(가만히 누름, 이동 허용 10pt)가 Reorder Mode를 활성화하며 이때 **가벼운 Impact Haptic 한 번**(`sensoryFeedback(.impact(weight: .light))`)이 울리고 해당 Clip이 선택되며 살짝 들린다(Scale 1.05 + 부드러운 그림자, Opacity 유지, 이웃 위에 표시; 이 Lift는 Drag 중에만 존재하고 Drop 시 정확한 44 × 78 Cell Geometry로 돌아온다 — Glow / Pulse 없음). 이어지는 Horizontal Drag에서 Clip은 손가락을 따라가고(Timeline 양 끝 밖으로는 나가지 않음) 들린 Clip의 중심이 다른 Slot의 중점을 지날 때 이웃 Clip이 부드럽게 자리를 내준다(STEP 9.1 Motion: Reflow `.spring(response: 0.28, dampingFraction: 0.86)`, Drop / Cancel Settle `.spring(response: 0.24, dampingFraction: 0.9)`, Lift Scale / Shadow만 `.spring(response: 0.2, dampingFraction: 0.9)`; Drag 위치 자체는 절대 Animate하지 않아 손가락과 1:1이며 Reduce Motion 시 Animation 없음). 별도의 삽입 Bar / Badge / 번호는 없으며 Live Preview Order 자체가 Insertion Feedback이다. Drop은 최종 순서를 한 번 Commit하고 Autosave한다(Save Button 없음, 완료 Haptic 없음); 같은 자리에 놓으면 아무것도 쓰지 않는다. Drag 중 Timeline Viewport 좌우 **32pt** Edge Band에 손가락이 머물면 Timeline이 ≈180pt/s로 점진적으로 Auto-scroll하며 Band를 벗어나거나 끝에 닿으면 멈춘다(Drag 중 수동 Scroll은 비활성). 선택은 Clip Identity를 따른다 — Drop 후에도 옮긴 Clip이 선택 상태이며 Thumbnail은 Clip과 함께 이동한다(Regeneration 없음). Cancel / Interruption / 화면 이탈 시 Committed Order로 복귀하고 저장하지 않는다. 저장 실패 시 순서를 되돌린 뒤 Native Alert `순서를 저장하지 못했어요.` / `다시 시도해주세요.` / `확인`을 보여준다(Undo 없음, 다시 Reorder). Accessibility(VoiceOver / Switch Control)는 각 Cell의 Custom Action `앞으로 이동`(N → N-1, 첫 Clip에는 없음) / `뒤로 이동`(N → N+1, 마지막 Clip에는 없음)으로 같은 Reorder / Autosave 경로를 사용하며 성공 시 `N번째 위치로 이동`을 짧게 Announce한다.

Phase 5 STEP 11 Add Clips(V4.1 Timeline, ADR-037, 2026-09-16 구현 — Physical Review Pending): Dock Leading의 `+`(승인된 40pt Circle, 44pt Target, Accessibility Label `클립 추가`, Hint `사진 보관함에서 프로젝트에 클립을 추가합니다.`)는 이제 Production Action이며 Timeline은 그 바로 뒤(Leading Inset 10 + 40 + 8)에서 시작한다. 탭 → System PhotosPicker(Video, 다중 선택) → 전부 Phase-5-ready일 때만 현재 Project 끝에 Picker 순서로 Append, 첫 번째 새 Clip 선택, Total 증가, Undo Enabled. Batch가 진행되는 동안 `+`는 Disabled + Progress 표시이고 Reorder / Delete / Undo / Redo도 Disabled다. Cancel은 조용히 아무것도 바꾸지 않으며 Non-ready / Invalid / Storage / 실패는 Select Clips와 같은 Typed Alert 한 번(공통 `ProjectMediaValidationCopy`: 5초 초과는 `영상이 너무 길어요` / `5초 이하의 영상을 선택해주세요.` — 5초 상한은 Mellow 제품 규칙이므로 "현재는" 같은 일시적 표현을 쓰지 않는다; HDR / 해상도 / Frame-rate / Orientation은 이후 Import 준비 대상이므로 "바로 사용할 수 없어요" 표현 유지 —, `클립을 추가하지 못했어요.`)으로 끝나고 Project는 그대로다. 한 Picker Session = 한 History Entry: Undo는 새 Clip 전부를 제거하고 Add 이전 Selection을 복원하며(Media는 보존), Redo는 같은 Clip을 같은 순서로 되살리고 첫 새 Clip을 다시 선택한다(Picker 재표시 없음). VoiceOver: 성공 시 `클립 N개를 추가했어요.`, Undo / Redo Hint `클립 추가 실행 취소` / `클립 추가 다시 실행`.

Phase 5 STEP 10 Delete + Session Undo / Redo(V4.1 Timeline, ADR-038, 2026-09-16 구현 — Physical Review Pending): Delete는 ADR-034 §3의 "선택 Clip의 명시적 Action"으로, Dock 우측 열에서 `Total` 아래 Bottom-trailing에 놓인 Compact `trash` Control(44 × 44pt, Accessibility Label `클립 삭제`)이다 — Thumbnail 위에 Trash를 두지 않고 Long Press Reorder와 섞지 않으며 Drag 중 / 저장 중 / 선택 없음이면 Disabled(흐림 + `사용할 수 없음`). 확인 Dialog는 없고(DESIGN §21) Undo가 안전 장치다. 탭 즉시 해당 Clip이 Timeline에서 사라지고 이웃이 자리를 채우며 Total이 줄고 Selection은 결정적으로 이동한다 — 삭제 Clip이 선택 상태였다면 같은 위치로 들어온 다음 Clip, 없으면 이전 Clip, 없으면 선택 없음; 선택 상태가 아니었다면 Selection 유지. **Undo / Redo는 Navigation Bar 우상단의 상시 Control**(`Back  Project  Undo Redo`; `arrow.uturn.backward` / `arrow.uturn.forward`, Symbol만, Native iOS 26 Glass Group, Accessibility Label `실행 취소` / `다시 실행`, Hint는 대상 편집을 말함 — `클립 삭제 실행 취소`, `클립 순서 변경 다시 실행`)로, History가 없거나 Commit / Drag 중이면 Disabled(System Disabled Tint, 어두운 Bar 위에서도 식별 가능)이며 중앙 `Project` Title, Preview Canvas, Dock Geometry를 건드리지 않는다. Delete Snackbar / Toast는 없다. History는 시간순 LIFO(Reorder + Delete, 이후 모든 편집)이고 Editor Session-local이다 — Editor를 떠나거나 재실행하면 Project는 마지막 Autosave 상태 그대로, Undo / Redo는 Disabled. Undo는 편집 전 State(Clip 집합 + Selection)를, Redo는 편집 후 State를 되돌려 각각 한 번 Autosave한다; Undo 뒤 새 편집은 Redo를 버린다. Selection: Reorder Undo는 옮긴 Clip을 유지, Delete Undo는 삭제했던 Clip을 다시 선택, Delete Redo는 삭제 직후 Selection을 복원, 유효하지 않으면 첫 Clip. 마지막 Clip을 삭제하면 Editor는 정상 빈 상태(ADR-026 Valid Draft)로 남는다 — Dock 유지, `Total 0.0s`, Preview `no clip selected`, Trash Disabled, Undo Enabled; Project를 자동 삭제하지 않는다. 저장 실패 Alert: Delete `클립을 삭제하지 못했어요.`, Undo `실행 취소하지 못했어요.`, Redo `다시 실행하지 못했어요.`(`다시 시도해주세요.` / `확인`) — 실패한 편집은 History에 들어가지 않고 실패한 Undo / Redo는 State와 Stack을 바꾸지 않는다. VoiceOver Announce: 삭제 후 `클립을 삭제했어요. 실행 취소할 수 있어요.`, Undo / Redo 후 `클립 삭제 실행 취소했어요.` / `클립 순서 변경 다시 실행했어요.`. Timer / Undo Window / Expiry Cleanup은 없다(ADR-038로 폐기). Phase 5 STEP 12A(ADR-039): Editor를 떠나는 Back은 항상 즉시 완료되고 Pending Clip의 물리 정리는 그 뒤 비동기로 진행되며, 정리 진행 / 실패를 보여주는 UI·Alert·Snackbar는 없다. 정리가 아직 진행 중일 때 같은 Project를 다시 열면 기존 Loading 상태(`Loading project`)가 잠깐 더 유지될 뿐 새 표현은 없다.

Phase 5 STEP 13 Unavailable Clip + Replace(V4.1 Timeline, ADR-040, 2026-09-16 구현 — Physical Review Pending): **Timeline Cell** — Project-owned 파일이 없는 Active Clip은 같은 44 × 78 Geometry, 같은 위치, 같은 Duration Tag, 같은 Signature Orange Selected Outline을 유지한 채 Neutral Dark Surface 위에 Secondary `video.slash` Glyph만 보인다(빨간 / Destructive 처리 없음, Cell 안 Text 없음, Geometry 확장 없음). 기존 Thumbnail 생성 실패 Cell(`film`)과는 다른 상태이며 서로 바꿔 쓰지 않는다. Accessibility Label은 `Clip N of M, X.Xs, unavailable`(Thumbnail 실패는 `…, thumbnail unavailable` 그대로), Selected Value / `앞으로 이동` / `뒤로 이동`은 그대로다. **Preview Shell** — 선택된 Clip이 Unavailable이면 기존 Full Preview Canvas(같은 Black, Card 없음) 중앙에 Secondary `video.slash`(30pt) → Headline `클립을 사용할 수 없어요`(Primary) → Subheadline `파일을 찾을 수 없어요.`(Secondary) → Capsule Button `클립 교체`(Dock Control Surface, Primary Text, 최소 44pt Target, Accessibility Hint `사진 보관함에서 이 클립을 교체합니다.`)를 세로로 보인다. Editor를 열 때 Alert / Modal은 없고 여러 Clip이 Unavailable이어도 각각 선택했을 때만 그 Shell을 본다. Healthy Clip 선택 시 Shell / Replace는 없다(Preview Accessibility Label `Preview, selected clip N`; Unavailable이면 `…, unavailable`). **Delete 관계** — Delete는 계속 Dock 우하단 Trash 하나이며 Shell에 두 번째 Delete를 두지 않는다. **Replace** — 탭 → System PhotosPicker(Video, 정확히 1개 선택) → Phase-5-ready이면 같은 자리에 새 Clip(새 Thumbnail, 자기 Duration), 새 Clip 선택, Total 갱신, Undo Enabled(Hint `클립 교체 실행 취소` / `클립 교체 다시 실행`), VoiceOver `클립을 교체했어요. 실행 취소할 수 있어요.`. 진행 중에는 Replace Button이 Progress를 보이고 `+` / Trash / Undo / Redo / Reorder가 Disabled다. Cancel은 조용히 아무것도 바꾸지 않고, Non-ready / Invalid / Storage는 Add와 같은 Typed Alert(`영상이 너무 길어요` / `5초 이하의 영상을 선택해주세요.` 등), 그 밖의 실패는 `클립을 교체하지 못했어요` / `다시 시도해주세요. 프로젝트는 그대로 있어요.` / `확인`이며 Unavailable Clip은 그대로다. Undo Replace는 Unavailable Clip과 Shell을 되돌리고 Redo는 Picker 없이 같은 새 Clip을 되돌린다. Unavailable Clip의 Reorder / Delete / Undo / Redo는 STEP 9 / 10과 동일하다.
...
Drag 중 현재 Clip의 위치와 삽입될 위치를 명확하게 표시한다(V4.1 구현: 들린 Clip + 이웃 Reflow가 삽입 위치를 나타내며 별도 Indicator는 없다 — 19절 STEP 9 Reorder Interaction).
...
ADR-038(2026-09-16): Undo Action은 Snackbar가 아니라 Navigation Bar 우상단의 상시 Undo / Redo Session History(`실행 취소` / `다시 실행`)로 제공하며 위 `Clip removed · Undo` 예시는 당시 기록이다. Delete Control 배치와 Selection Fallback은 19절 STEP 10 항목을 따른다. Undo Window Duration이라는 결정 항목은 더 이상 존재하지 않는다.
...
Clip Delete / Undo Presentation은 ADR-038에 따라 Editor Session Undo / Redo History(시간순 LIFO, Session-local, Process 종료 후 미유지, 동일 Clip Identity / Media 복원)를 따르며 "가장 최근 삭제 한 건" 제한과 Snackbar는 폐기되었다.
...
- Add Clip Action의 위치
- Unavailable Clip의 Exact Icon, Placeholder, Label, Color와 Replace / Delete Button Hierarchy — Resolved by ADR-040 (19절 STEP 13)
- Replacement의 Trim, Framing, Transform, Thumbnail Reset을 사용자에게 알리는 방식 — Resolved by ADR-040: Phase 5에는 사용자 Trim / Framing이 없어 안내할 Reset이 없고 새 Clip은 새 Thumbnail을 갖는다
