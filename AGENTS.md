...
- V1 Photos import accepts only H.264 (`avc1`/`avc3`) or HEVC (`hvc1`/`hev1`) video codec families; ProRes, ProRes RAW, Motion JPEG, other and unknown codecs are excluded during preflight (after the container check, before orientation), never normalized, and reuse the existing invalid/unsupported copy (ADR-046). Mellow's own camera must explicitly request H.264 QuickTime SDR output and fail setup safely if unavailable — no silent HEVC/ProRes/default fallback and no codec settings UI (ADR-046).
- Working media is never upscaled: output raster = `scale = min(1.0, 1080 / width, 1920 / height)` applied to the portrait presentation raster, each edge floored to even, no minimum output size, no resolution selector or upscale option (ADR-047). Interrupted normalization is never resumed after process termination — no durable resumable operation ID, checkpoint or background continuation; the workspace UUID is ephemeral, the existing startup workspace sweep cleans abandoned work, and the user re-initiates the import (ADR-047).
- V1 Photos import accepts only sources whose actual container is QuickTime Movie (H.264 or HEVC inside); MP4 / ISO BMFF, other and unknown containers are excluded during preflight by reliable container inspection (never by file extension alone), with no remux or container conversion (ADR-044).
...
- The Phase 6 spike evidence branch `spike/06-media-technical-gate` (baseline `04d83612`) is a disposable diagnostic harness: never merge it into `main`, never rebase or cherry-pick it, and never rename/copy its files into production — reimplement the ADR-045 contracts as new abstractions.
