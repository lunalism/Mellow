...
## 확정된 설계 결정
- 촬영 스펙은 세션 단위로 고정. 30fps·HEVC·AAC 공통.
  기본값이 HEVC이며 개입 없이 클립 간 스펙이 일치함을 1-3 실측으로 확인했다.
  v0.2에서 재인코딩(자막·BGM)을 붙일 때 HEVC 소스의 처리를 다시 검토한다.
- 세로 세션 1080×1920, 가로 세션 1920×1080은 렌더링 결과 크기를 가리킨다.
  AVCaptureMovieFileOutput은 회전을 QuickTime 트랙 매트릭스로 기록하므로
  인코딩 해상도는 방향과 무관하게 1920×1080이고, 세로/가로 구분은
  preferredTransform에 담긴다. 1-11 실측으로 확인했다 —
  세로로 찍은 클립도 encoded 해상도가 1920×1080이었다.
- 한 세션 안의 클립은 전부 동일 스펙이어야 재인코딩 없는 병합이 가능하다.
  preferredTransform도 같아야 한다.
