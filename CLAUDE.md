...
- **`FrameProcessor.crossfade` 은퇴: RESOLVED (2026-07-22, 기획 결정 확정 후 삭제).** 결정: **영상 녹화 중 필터 잠금** — 브이로그 테이크의 무결성을 위해 녹화 중 필터 변경 불가. 스틸은 즉시 스왑(L3 Decision B)으로 필터 변경 자유, 영상은 **녹화 중에만** 필터 잠금 — 어느 모드에도 크로스페이드형 전환 **효과**가 필요 없다 → 호출처 0이던 `FrameProcessor.crossfade` 삭제(전용 헬퍼 없음, Spec §4.2는 은퇴 노트로 기록).
- 낮은 우선순위 백로그 — **LUT 프리페치 패스스루 창의 WYSIWYG 위반 (Codex 적대 리뷰 2026-07-22 발견):** 선택 slug의 LUT가 아직 로드되지 않은 동안 프리뷰는 패스스루로 표시되는데, 그 상태에서 촬영하면 slug이 그대로 저장돼 썸네일·상세·익스포트는 LUT 적용본으로 렌더된다 — 프리뷰(무필터)와 결과물(필터됨)이 갈라지는 좁은 창. 수정 = 캡처 또는 필터 선택을 LUT 레디니스로 게이트 — **동작 변경이라 기획 결정 필요, 임의 구현 금지.** .lutbin 프리컴파일 이후 이 창은 매우 짧을 것으로 예상. **Phase 2 설계 입력(구현 과제 아님):** 녹화 시작이 LUT 레디니스의 자연스러운 게이트 — 녹화 버튼을 선택 slug의 LUT 로드 완료로 게이트하면(녹화 중 필터 잠금 결정과 결합) 영상 쪽 프리페치 창 WYSIWYG 갭도 함께 닫힌다.
