...
- `transaction { ... }` 안에서 블록이 던지면 throw는 전파되지만 블록이 한
  ...
  이름값을 못 한다
- `rollback()`은 **스토어 기준으로는 제대로 되돌린다** — 이후 `save()`를
  불러도 그 변경은 들어가지 않는다. 그러나 **우리가 들고 있는 `@Model`
  객체는 바뀐 값을 그대로 들고 있다**(stale). 실측: `title`을 바꾸고
...
**`ClipStore`도 같은 모양이다** (`insert` → `save()` → 실패 시 `rollback()`).
아직 손대지 않았다.
